function create_results_insights_d(instance, TP_targets, create_figures, ENV)
rng(14);
...
parts_nice = ['None', cellfun(@(c)['M_', num2str(c)],num2cell(1:machines),'UniformOutput',false)];
...
table_results_columns = {'TP^T', 'Bottleneck', '\# solvable', 'mean costs', 'TBC (mean)', 'TBC (std. error)', 'TBC (min)', 'TBC (max)', 'TCU (mean)', 'TCU (std. error)', 'TCU (min)', 'TCU (max)'};
...
table_results_columns_allocation = ['TP^T', 'Bottleneck', cellfun(@(c)['C_', num2str(c)],num2cell(1:(machines-1)),'UniformOutput',false), cellfun(@(c)['Q_', num2str(c)],num2cell(1:machines),'UniformOutput',false)];
