function generate_instances_insights_d_bottlenecks(ENV)
rng(4);
...
%% settings
filename = 'insights_d_bottlenecks';
number_instances = 100;
machines = 5;

deviation_mu_min = 0.05;
deviation_mu_max = 0.1;
deviation_mu_type = 'worse';
deviation_p_min = 0.1;
deviation_p_max = 0.2;
deviation_p_type = 'worse';
deviation_gamma_min = 0.1;
deviation_gamma_max = 0.2;
deviation_gamma_type = 'worse';
...
number_instances_total = (machines + 1) * number_instances;
...
instances_bottleneck = null(number_instances_total, 1);
...
% mu
random_numbers = rand(number_instances, 1);
...
%% combine everything
for k = 1:machines
    index = k * number_instances + 1;
    instances_mu(index:(index + number_instances - 1), k) = dev_rand_mu;
    instances_p(index:(index + number_instances - 1), k) = dev_rand_p;
    instances_gamma(index:(index + number_instances - 1), k) = dev_rand_gamma;
    instances_bottleneck(index:(index + number_instances - 1), 1) = k * ones(number_instances, 1);
end
