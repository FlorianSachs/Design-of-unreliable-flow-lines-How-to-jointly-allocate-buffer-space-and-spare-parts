function generate_instances_insights_c_costs_one_spare(ENV)
rng(3);
...
%% settings
filename = 'insights_c_costs_one_spare';
...
for k = 1:length(base_costs_spares)
    ...
    instances_costs_spares(index:(index + number_instances - 1), 3) = base_costs_spares(k) .* ones(number_instances, 1);
end
