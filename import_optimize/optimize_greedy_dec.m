function [C, Q, TP_current, costs_current, counter_iterations, counter_evaluations, counter_problems] = optimize_greedy_dec(machines, C_min, Q_min, C_max, Q_max, mu, p, gamma, TP_target, cost_buffers, cost_spares, C_start, Q_start, outputdir, filename, check_buffers, check_spares, allow_Q_min_adjustements, filename_to_load)
%optimize_greedy_dec uses greedy decrease steps to reach target
%   Buffers or spare part inventories are decreased to reach the target
...
while (TP_current > TP_target)
    ...
    decrease_steps = ones(1, 2*machines-1);

    % one buffer place or one spare part less
    ...
    for i = 1:2*machines-1
        ...
        for decrease_step = 1:decrease_end
            n_tmp = C;
            s_tmp = Q;

            % decrease n or s
            decrease_steps(i) = decrease_step;
            if (i < machines)
                n_tmp(i) = n_tmp(i) - decrease_step;
                if (n_tmp(i) < min_vector(i))
                    TP(i) = NaN;
                    costs(i) = NaN;
                    continue
                end
            else
                s_tmp(i - machines + 1) = s_tmp(i - machines + 1) - decrease_step;
                if (s_tmp(i - machines + 1) < min_vector(i))
                    TP(i) = NaN;
                    costs(i) = NaN;
                    continue
                end
    ...
    % TP constraint violated
    TP(TP < TP_target) = NaN;

    % decreased buffer/spares but increased TP
    ...
    if isnan(val)
        table_values_rows(counter_iterations, 1:(8*machines)) = [TP_current NaN NaN NaN C Q Delta TP decrease_steps];
        break;
    end
    ...
    if (pos < machines)
        C(pos) = C(pos) - decrease_steps(pos);
        posN = pos;
        posS = NaN;
    else
        Q(pos - machines + 1) = Q(pos - machines + 1) - decrease_steps(pos);
        posN = NaN;
        posS = pos - machines + 1;
    end

    % output
    table_values_rows(counter_iterations, 1:(8*machines)) = [TP_old TP_current posN posS C Q Delta TP decrease_steps];
